...
1. **Primary engine** — *(design updated 2026-06-29, dataset question reopened 2026-07-07, KD directions re-locked 2026-07-07)* Two-step training (Step 1 KD-pretrain on a public corpus → Step 2 gold-CE FT on private `Qᵢ`, sequential per `system_architecture.md` §5.6) + FedAvg remains the target **full-method architecture**:
   ...
   - **What's deferred (2026-07-07):** which corpus is the KD step's public data (BIRD dropped; no replacement locked). **Until it's picked, run the PoC (`KD_PLAN.md` §PoC) directly on Spider** — three arms from base on identical data (`poc_ft` / `poc_rkd` / `poc_kid`), no two-step split, no federation — to validate the KD signal itself before wiring the full design to any specific corpus.
...
8. **KD comparison hygiene** — *(revised 2026-07-07 (2), CoT dropped)* Full control ladder (`fedavg → fedavg_pub → fedkd_rkd → fedkd`) is deferred until the public corpus is picked (see dec.4). **For now:** the PoC compares, on identical Spider data, from the base model: `poc_ft` (gold CE, no teacher) vs `poc_rkd` (`CE + RKL` on gold `y`) vs `poc_kid` (`CE + RKL` on imperfect `ŷ`, ρ=0.2 Random). Each rung adds one ingredient: `poc_rkd − poc_ft` = teacher-logit value, `poc_kid − poc_rkd` = imperfect-data value. Old E0.3 blocker resolved — KID mask-fill mechanics pinned from [10] (`KD_PLAN.md` §mechanics). DP claim scoped to "DP-ready + (ε, EX) curve" (K=3 → no strong-ε promise). Staged run order = `KD_PLAN.md` §PoC.
