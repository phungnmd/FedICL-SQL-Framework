...
| 2026-08-31 | Pulled P1.3b result `ce93c79` and passed artifact review. FL and FedLS records have identical client-training and aggregation summaries: 8,659 examples, 543 updates, five fresh clients, plaintext weighted factor FedAvg, `min_cosine=0.407722`, and `noop_suspect=False`. The FedLS record adds only one fresh RKL server stage over the canonical 3,873-row Qwen teacher-selected pool, initialized from the shared aggregate; it processed all 3,873 rows in 6,896.5 s with `resumed_from=0`. Training records do not contain EX, so no stronger-skew method claim is made yet. Activated P1.3c paired T1 evaluation on the exact 1,034-row committed Spider test set, with tracked-diff plus platform-stable Git-blob identity preflight, a dedicated resume root, and separate allowlisted publication. The frozen gate remains `>=+2.0` EX, corrections greater than regressions, and no execution-error increase. |
