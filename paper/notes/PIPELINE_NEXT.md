...
| P2.3a | Implement composable stage chains (`run.py stage`) | reviewed through `014b118`; smoke running (user report, 2026-09-16) |
...
Smoke is currently running (user report, 2026-09-16). Do not stop it or pull
code to use these commands: they use the already reviewed `014b118` CLI.
Keep smoke output; its compact publication is deferred to the final command
below. No manual publication or pause is required between smoke and full jobs.

Start the following in two terminals in the **same Windows login session**.
The two jobs train concurrently; each then automatically evaluates Spider,
