...
Next: the four phases under *Active P2.3 commands*, in order. `A→A` and
...
```powershell
$ErrorActionPreference='Stop'; $Scope=@('fedicl_sql','experiments','scripts','tests','processed_data/protocol_v2','pyproject.toml','uv.lock'); $Dirty=@(git status --porcelain --untracked-files=no -- $Scope); if ($Dirty.Count -ne 0) { $Dirty | ForEach-Object { Write-Host $_ }; throw 'Scientific scope is dirty; review before pull' }; git pull --ff-only origin main; if ($LASTEXITCODE -ne 0) { throw 'Pull failed' }; git merge-base --is-ancestor 014b118 HEAD; if ($LASTEXITCODE -ne 0) { throw 'Checkout does not contain stage-chain commit 014b118' }; Write-Host 'P2.3 code ready; run smoke next, then open the two training terminals'
```
...
**Publish the smoke record before starting either full job** (no experiment
process may still use this worktree). Keep the local smoke adapters for
inspection/resume; they are not paper accuracy results. Only these two compact
result files are committed; no automatic deletion is needed.

```powershell
$ErrorActionPreference='Stop'; $O='artifacts/protocol_v2/p23_smoke/a_k_a_steps2_s0'; $St=Get-Content -LiteralPath "$O/stage.json" -Raw | ConvertFrom-Json; if ($St.version -ne 2 -or $St.chain -ne 'A>K[fkl]>A') { throw 'Unexpected smoke contract' }; if (@(git diff --cached --name-only).Count -ne 0) { throw 'Index is not empty' }; $Rows=@(Get-ChildItem -LiteralPath 'experiments/federated/results' -Directory -Filter 'federated_stage__*' | Where-Object { $P=Join-Path $_.FullName 'metrics.json'; try { $M=Get-Content -LiteralPath $P -Raw | ConvertFrom-Json; $M.stage_id -eq $St.stage_id -and $M.stage -eq 'p23_smoke_a_k_a' } catch { $false } }); if ($Rows.Count -ne 1) { throw 'Expected exactly one completed smoke result' }; $C=Get-Content -LiteralPath (Join-Path $Rows[0].FullName 'config.json') -Raw | ConvertFrom-Json; if ($C.out -ne $O -or $C.client_max_steps -ne 2) { throw 'Smoke result config mismatch' }; $Files=@("experiments/federated/results/$($Rows[0].Name)/config.json","experiments/federated/results/$($Rows[0].Name)/metrics.json") | Sort-Object; git add -- $Files; if ($LASTEXITCODE -ne 0) { throw 'Smoke result staging failed' }; $Staged=@(git diff --cached --name-only | Sort-Object); if ($Staged.Count -gt 0) { if (@(Compare-Object $Files $Staged).Count -ne 0) { throw 'Smoke staged allowlist mismatch' }; git commit -m 'results: record P2.3 stage-chain smoke'; if ($LASTEXITCODE -ne 0) { throw 'Smoke result commit failed' } }; git push origin main; if ($LASTEXITCODE -ne 0) { throw 'Smoke result push failed' }; git log -1 --oneline
```

**2. Train both arms (two independent terminals).** Output-disjoint and
exact-rerun safe; do not pull or commit until both exit.
Both jobs are student-only (no new teacher logits or KD stage), with one extra
private epoch per client and a final FedAvg. If only one GPU is free, run
`A>K[fkl]>A` first, then `A>A`; change only `CUDA_VISIBLE_DEVICES`, not roots
or scientific flags. Do not run two invocations against the same output root.

GPU 0, matched no-KD control `A>A` (parent: Pure FL T1):

```powershell
$ErrorActionPreference='Stop'; git merge-base --is-ancestor 014b118 HEAD; if ($LASTEXITCODE -ne 0) { throw 'Sync reviewed P2.3 code before starting any GPU job' }; $env:CUDA_VISIBLE_DEVICES='0'; $env:PYTHONUTF8='1'; $S='processed_data/protocol_v2/SPIDER/processed_train8659_dev1034/federated_noniid/alpha_0.5/k5'; $R='experiments/federated/results/federated__fedavg__s0__935d572565cc__154540d3__r1'; $O='artifacts/protocol_v2/p23_spider_private_terminal/a_a_s0'; uv run python experiments/federated/run.py stage private --aggregation-protocol plaintext --parent-result $R --split-dir $S --n-clients 5 --local-epochs 1 --client-train-k 0 --client-dataset-profile spider --model Qwen/Qwen2.5-1.5B-Instruct --lora-r 16 --lr 0.0002 --max-len 7168 --truncation-policy error --gradient-checkpointing --batch-size 1 --grad-accum 16 --save-steps 200 --seed 0 --stage p23_spider_private_a_a --out $O; if ($LASTEXITCODE -ne 0) { throw 'A>A stopped; rerun this exact line' }; $St=Get-Content -LiteralPath "$O/stage.json" -Raw | ConvertFrom-Json; if ($St.version -ne 2 -or $St.chain -ne 'A>A' -or $St.contains_kd -or -not (Test-Path -LiteralPath "$O/fedavg_adapter/adapter_config.json")) { throw "Unexpected A>A contract: chain=$($St.chain)" }; Write-Host 'GPU-0 complete: A>A trained; wait for GPU-1 before publication'
```

GPU 1, candidate `A>K[fkl]>A` (parent: Hinton T1):

```powershell
$ErrorActionPreference='Stop'; git merge-base --is-ancestor 014b118 HEAD; if ($LASTEXITCODE -ne 0) { throw 'Sync reviewed P2.3 code before starting any GPU job' }; $env:CUDA_VISIBLE_DEVICES='1'; $env:PYTHONUTF8='1'; $S='processed_data/protocol_v2/SPIDER/processed_train8659_dev1034/federated_noniid/alpha_0.5/k5'; $R='experiments/federated/results/federated__fedkd__s0__55c03ff12654__8a1e6694__r1'; $O='artifacts/protocol_v2/p23_spider_private_terminal/a_k_a_s0'; uv run python experiments/federated/run.py stage private --aggregation-protocol plaintext --parent-result $R --split-dir $S --n-clients 5 --local-epochs 1 --client-train-k 0 --client-dataset-profile spider --model Qwen/Qwen2.5-1.5B-Instruct --lora-r 16 --lr 0.0002 --max-len 7168 --truncation-policy error --gradient-checkpointing --batch-size 1 --grad-accum 16 --save-steps 200 --seed 0 --stage p23_spider_private_a_k_a --out $O; if ($LASTEXITCODE -ne 0) { throw 'A>K>A stopped; rerun this exact line' }; $St=Get-Content -LiteralPath "$O/stage.json" -Raw | ConvertFrom-Json; if ($St.version -ne 2 -or $St.chain -ne 'A>K[fkl]>A' -or -not $St.contains_kd -or -not (Test-Path -LiteralPath "$O/fedavg_adapter/adapter_config.json")) { throw "Unexpected A>K>A contract: chain=$($St.chain)" }; Write-Host 'GPU-1 complete: A>K[fkl]>A trained; wait for GPU-0 before publication'
```
...
Run the separate publication command in phase 4 after evaluation completes.
The two training arms share that publication step, intentionally delayed until
both have exited so no running process records a mid-run Git commit.
