# ARCHIVED — FedLS-SQL protocol-v1 lab log (BIRD evidence omitted)
...
| 2026-08-30 | Pulled P1.5d result `9537103` and closed the FedProx trajectory plus the proposed FedLS-FedProx interaction screen. Against identical pure-FL rows, FedProx scores 55.80 vs 56.67 EX at T1 (`26/35` corrections/regressions, exact `p=0.306`, 249/241 errors) and 59.96 vs 62.19 at T2 (`19/42`, `p=0.00444`, 214/202 errors). With T3 already `-1.55`, the round-matched deltas are `-0.87/-2.22/-1.55`; there is no early FedProx advantage. FedProx itself still improves T1→T2→T3, so the conclusion is not training collapse but consistent inferiority to matched FedAvg under fixed `mu=0.01`. Do not run combined FedLS-FedProx, OOD, or coefficient tuning; proceed to P1.3 stronger-skew. |
| 2026-08-30 | Completed P1.3a at nested code/data commit `e97583d` with 337 tests passing. The grouped-domain Dirichlet `alpha=0.1, K=5, seed=0` candidate has the identical 8,659-row multiset and zero cross-client DB overlap relative to the main `alpha=0.5` split. Mean pairwise domain JSD rises `0.5266→0.8047` bits and mean client domain entropy falls `3.0291→2.1834`; all clients retain 1,097–2,061 rows. The size ratio falls `3.02x→1.88x`, so P1.3 is explicitly stronger semantic-domain skew, not stronger quantity skew. Activated P1.3b shared-client FL/FedLS T1 training with separate allowlisted publication command. P1.3c evaluation waits for artifact review; its frozen T3-promotion gate is `>=+2.0` Spider EX, more paired corrections than regressions, and no increase in execution errors. |
| 2026-08-31 | Corrected the P1.3b cleanliness preflight after the shared GPU server exposed unrelated tracked processed-data edits and 61 untracked historical datasets/caches. Untracked files were never the cause because the old check excluded them; the blocker was a global tracked-worktree check. P1.3b now fails only on an explicit scientific scope: `fedicl_sql`, `experiments/federated`, dependency locks, the committed alpha=0.1 split/audit, and the exact BIRD public pool. Existing modified centralized Spider files are outside training scope and are preserved, but P1.3c must audit/reconcile them because they are evaluation inputs. No server-local dataset is deleted or silently restored. |
| 2026-08-31 | P1.3b completed the shared client/FedAvg stage and the 6,898-second FedLS RKL server stage, then failed only its documentation-level acceptance check. `setup_recipe()` canonicalizes `split_dir` with `Path.resolve()`, while the command compared it to relative `$D`; training, manifests, and adapters are unaffected. Added a standalone corrected verifier using `(Resolve-Path $D).Path`. Do not rerun GPU training; verify, publish the compact FL/FedLS result files, then stop for artifact review. |
...
| 2026-09-02 | Promoted P1.9 ahead of seed 2 and manuscript freeze to resolve the main remaining KD attribution gap. Headline T3 and stronger-skew T3 both compare FL with CE+RKL, so they establish the full server stage but cannot assign its gain to logits. Existing T1 evidence gives a positive but non-robust RKL increment: Qwen seed 0 `+2.03` EX, three-seed mean `+1.71` with `p=0.165`, and Gemma `+0.19` with `p=0.916`. P1.9 uses the current stronger-skew lineage because its exact round-one shared FedAvg aggregate and full CE+RKL T3 control are current, audited, and free of legacy-setup migration. P1.9a reuses that shared private-client/FedAvg stage, then runs verified teacher-target CE only through T3; P1.9b will pair its endpoint against existing CE+RKL T3. This estimates the total multi-round effect of including RKL, including downstream client-trajectory changes. Do not freeze RKL as a key contribution before this gate. |
