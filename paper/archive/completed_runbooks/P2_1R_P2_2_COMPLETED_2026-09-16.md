# Protocol-v2 completed runbooks — P2.1R/P2.2

> Archived 2026-09-16 from `paper/notes/PIPELINE_NEXT.md` at outer commit `3caddb2`.
> Historical reference only — not the active run queue. Commands below are
> retained verbatim, including superseded sequencing and cleanup instructions;
> do not execute them as current guidance. Existing results remain valid.

Current commands: [PIPELINE_NEXT.md](../../notes/PIPELINE_NEXT.md).
Current results/adapters: [VALID_RESULTS_AND_ADAPTERS.md](../../notes/VALID_RESULTS_AND_ADAPTERS.md).
Dataset/direction definitions: [system_architecture.md](../../notes/system_architecture.md).

Preserved scope: public-gold CE evaluation; both private/public directions;
teacher prerequisites and Hinton cache; headline and matched-ablation commands;
prior completion checks; the original rationale for terminal private consolidation.
Statements such as “next” or “run these” below describe the old queue, not a new task.
