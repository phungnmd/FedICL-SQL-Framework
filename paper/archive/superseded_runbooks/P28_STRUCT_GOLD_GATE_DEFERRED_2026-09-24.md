# FedLS-SQL — deferred P2.8 matched-gold structured gate

Preserved on 2026-09-24. The user chose to run the P2.9 terminal-retention
gate first, because P2.6 public endpoints show the flat SeqKD generalization
edge that plain terminal A erases. P2.8 is deferred, not closed: resume it from
the commands below after the P2.9 decision. Required nested commit remains
`6458b33` or a descendant; the P2.9 branch `experiment/terminal-retention`
descends from it. When resuming, run Step 0 against
`experiment/terminal-retention` (or its successor) instead of switching back to
`experiment/structured-rationale-kd`, so the retention code stays present.
